			...
			if frozen == false: #dit lijkt geen problemen te veroorzaken
				if player_chase:
					...
					if(player.position.x - position.x) < 0:
						...
						$AnimatedSprite2D/Sprite2D.position = Vector2(4, 18)
						$Enemy_Damage_Box/CollisionShape2D.position = Vector2(17,1)
					else:
						$AnimatedSprite2D.flip_h = false
						$AnimatedSprite2D/Sprite2D.position = Vector2(-2, 18)
						$Enemy_Damage_Box/CollisionShape2D.position = Vector2(-17,1)
				else:
...
func enemy():
	if isdying == false:
		...
		if inrealattackzone == true and not isdying:
...
func _on_real_zone_body_entered(body):
	if body.has_method("player"):
		inrealattackzone = true


func _on_real_zone_body_exited(body):
	if body.has_method("player"):
		inrealattackzone = false
